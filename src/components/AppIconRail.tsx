...
import { useEditorUiStore } from "../libs/store/editor-ui-store";
  ...
  <button
    className={cn(
      "flex flex-col items-center justify-center p-1 rounded-md transition-colors w-full aspect-square gap-1",
      {
        ...
        "text-muted-foreground hover:bg-panel-hover/20 hover:text-accent-foreground": !active,
      }
    )}
    onClick={onClick}
    title={label}
...
const AppIconRail: React.FC<IconRailProps> = ({ onViewChange }) => {
  const activeView = useEditorUiStore((state) => state.activeSideView);
  return (
